...
func testMap(){
	...
	func testChannelGoRoutine(){
		...
		func testChannelSelect(){
			...
			ch2 <- "A"
			// この状態ではch1にはデータがないのでdeadlockになる
			v1:= <- ch1
			v2:= <- ch2
			fmt.Println(v1)
			fmt.Println(v2)
		}
		func testChannelSelect2(){
			ch1 := make(chan int, 2)
			ch2 := make(chan string, 2)

			ch2 <- "A"
			//select文を使用すると、複数のチャネルのデータを受信することができる
			//select文は、複数のcase文を持つことができる
			//select文は、複数のcase文の中から、ランダムに一つを選択して実行する
			//select文は、デフォルトのcase文を持つことができる
			//select文は、デフォルトのcase文は、他のcase文が実行できない場合に実行される
			// この状態ではch1にはデータがないのでdeadlockになる
			select{
				//チャネルに対する操作のみを記述できる
			case v1:= <- ch1:
